...
export default function Selected() {
  ...
  return (
    ...
    `}>
      <h1 className='mx-auto p-2 text-red-500 text-8xl font-bold flex flex-row justify-center text-center border-b-2 border-red-800'>Quadríceps</h1>
      <ul>
      {exercisesArray.map((exercise) => (
        <li key={exercise} className={`
        exercise-${1} mb-3 text-xl justify-self text-center
        `}>
          <div className='w-fit m-5 display flex flex-row gap-8 p-6 justify-center items-center font-bold border-b-2 border-b-zinc-500 text-4xl'>
            {exercise}
            <CheckboxWithModal />
            <CheckboxWithModal />
            <CheckboxWithModal />
